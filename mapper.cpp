...
// PRG 16/32 KiB
// CHR 8 KiB
uint32_t Mapper0::MapReadPRG(uint16_t address, bool &fromRAM)
{
...
uint32_t Mapper0::MapWritePRG(uint16_t address, uint8_t data, bool &toRAM)
{
    if (numBanksPRG == 1)                                           // mapped PRG address == first 14 bits if 16 KiB (0x8000 - 0xBFFF mirrored 0xC000 - 0xFFFF)
        return address &= 0x3FFF;
    else if (numBanksPRG == 2)                                      // mapped PRG address == first 15 bits if 32 KiB (0x8000 - 0xFFFF)
...
uint32_t Mapper0::MapReadCHR(uint16_t address)
{
    return address;
}

uint32_t Mapper0::MapWriteCHR(uint16_t address)
{
    return address;
}
...
// PRG 256/512 KiB
// CHR 128 KiB
Mapper1::Mapper1(uint8_t numBanksPRG, uint8_t numBanksCHR) : Mapper(numBanksPRG, numBanksCHR)
{
    Reset();
}
...
uint32_t Mapper1::MapReadPRG(uint16_t address, bool &fromRAM)
{
    uint32_t mappedAddress = 0x00000000;

    if (PRGRAMEnable && address >= 0x6000 && address <= 0x7FFF)  // 8 KiB PRG RAM (optional)
    {
...
uint32_t Mapper1::MapWritePRG(uint16_t address, uint8_t data, bool &toRAM)
{
    static uint8_t shift;

    uint32_t mappedAddress = 0x00000000;

    if (PRGRAMEnable && address >= 0x6000 && address <= 0x7FFFF)  // 8 KiB PRG RAM (optional)
    {
        mappedAddress = address & 0x1FFF;                                                                        
        toRAM = true;
    }
        ...
        else
        {
            ...
            else
            {
                ...
                else if ((address & 0x4000) && (address & 0x2000))    // PRG bank: address 0xE000 - 0xFFFF, bit 14 == 1, bit 13 == 1
                {
                    ...
                    PRGRAMEnable = internalRegister & 0x10;
                }
            }
        }
    }
...
uint32_t Mapper1::MapReadCHR(uint16_t address)
{
    uint32_t mappedAddress = 0x00000000;
...
uint32_t Mapper1::MapWriteCHR(uint16_t address)
{
    uint32_t mappedAddress = 0x00000000;
...
// PRG 256/4096 KiB 
// CHR 8 KiB
uint32_t Mapper2::MapReadPRG(uint16_t address, bool &fromRAM)
{
...
uint32_t Mapper2::MapWritePRG(uint16_t address, uint8_t data, bool &toRAM)
{
...
uint32_t Mapper2::MapReadCHR(uint16_t address)
{
    return address;
}

uint32_t Mapper2::MapWriteCHR(uint16_t address)
{
    return address;
}
...
// PRG 16/32 KiB 
// CHR 32 KiB 
uint32_t Mapper3::MapReadPRG(uint16_t address, bool &fromRAM)
{
    if (numBanksPRG == 1)                                           // mapped PRG address == first 14 bits if 16 KiB (0x8000 - 0xBFFF mirrored 0xC000 - 0xFFFF)
        return address &= 0x3FFF;
    else if (numBanksPRG == 2)                                      // mapped PRG address == first 15 bits if 32 KiB (0x8000 - 0xFFFF)
...
uint32_t Mapper3::MapWritePRG(uint16_t address, uint8_t data, bool &toRAM)
{
...
uint32_t Mapper3::MapReadCHR(uint16_t address)
{
    return mappedCHRBank * 0x2000 + address;                        // mapped CHR address == index of CHR bank * 8 KiB + offset
}

uint32_t Mapper3::MapWriteCHR(uint16_t address)
{  
    return mappedCHRBank * 0x2000 + address;                        // mapped CHR address == index of CHR bank * 8 KiB + offset
}